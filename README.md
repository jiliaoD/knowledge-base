...
![问答页：四轮对话，包含带图回答、代词追问、换型号和库外问题兜底](docs/images/chat-demo.png)

上图是一段真实问答记录（未做任何美化）：

1. 问 `HAK180怎么烫金` —— 命中的内容全部来自手册切片，答案直接附带手册原图
2. 用代词追问 `它的耗材多少钱` —— 靠会话历史把"它"消解成 HAK180，手册里没有价格就直说没有
3. 换型号问 `华为显示器 B3-271Q 怎么调亮度` —— 商品名重新对齐后检索另一本手册
4. 问库里没有的 `小米15怎么截屏` —— 不硬答，直接说明未找到并要求补充型号
