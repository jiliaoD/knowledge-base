...
class NodeMDImg(BaseNode):
    ...
    def _step_4_upload_and_replace(
        ...
        # 2. 获取图片上传路径
        minio_dir = self.config.minio_image_dir
    ...
    def _upload_to_minio(self, minio_client: Minio, local_path: str, object_name: str) -> str | None:
        ...
        try:
            ...
            # 组织图片url
            url = f"http://{self.config.minio_endpoint}/{self.config.minio_bucket}/{object_name}"
            return url
    ...
    def _clean_minio_dir(self, minio_client: Minio, update_dir: str) -> None:
        ...
        try:
            # 1. 获取将要被删除的图片列表
            objects_to_delete = minio_client.list_objects(self.config.minio_bucket, update_dir, recursive=True)
    ...
    def _step_3_generate_summaries(self, doc_stem: str, target_images: List[Tuple[str, str, Tuple[str, str]]]) -> Dict[str, str]:
        ...
        for img_file, img_path, context in target_images:
            ...
            # 3.1 限速
            #     限速值来自配置（.env 的 VL_REQUESTS_PER_MINUTE），避免写死导致大批量导入过慢
            self._apply_api_rate_limit(request_deque, self.config.requests_per_minute, 60)
...
if __name__ == "__main__":
    ...
    md_path = r"D:\22857\Desktop\doc\output_md\hak180产品安全手册\hak180产品安全手册.md"
