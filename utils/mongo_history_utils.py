...
def get_recent_messages(session_id: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # 执行查询：先按时间戳"降序"取最近的N条，再反转成时间正序返回
        # 关键点：必须"先降序 + limit"才能拿到最近N条。
        # 原来写的是升序 + limit，取到的是会话里最旧的N条——长会话时会把会话开头
        # 当成"近期上下文"喂给大模型，导致追问能力退化。
        # find(query)：获取符合条件的游标（惰性加载，不立即查询）
        # sort("ts", DESCENDING)：按ts字段降序（从新到旧），这样limit截取到的才是最近的
        # limit(limit)：限制返回的最大条数
        cursor = mongo_tool.chat_message.find(query).sort("ts", DESCENDING).limit(limit)
        # 将游标转为列表，触发实际数据库查询
        messages = list(cursor)
        # 反转回时间正序（从旧到新），方便直接按对话顺序喂给LLM
        messages.reverse()
        # 返回查询结果列表
        return messages
    except Exception as e:
